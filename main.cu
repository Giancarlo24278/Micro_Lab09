...
void analyzeData(const std::vector<SensorData>& data, int intent, const std::string& query) {
    ...
    float max_val = std::max({total_luces, total_ac, total_riego,total_puerta, total_ascensor});
...
int main(){
    ...
    const std::string filename = "ConsumoCasaInteligente.csv";

    // Cargar datos CSV
    std::string csv_path = "ConsumoCasaInteligente.csv";
